...
const imageModules = import.meta.glob('../../assets/MediaGalleryImgs/*.webp', { eager: true });
const imageMap = Object.entries(imageModules).reduce((acc, [path, module]) => {
  const fileName = path.split('/').pop();
  acc.push({ image: module.default, title: fileName.split('.')[0] });
  return acc;
}, []);

const AwardsCarousel = ({ autoPlay = true, interval = 5000 }) => {
  const galleryItems = imageMap;
